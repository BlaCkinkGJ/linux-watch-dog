        ...
        if(base_entry->status.st_ino == check_entry->status.st_ino) {
            ...
            if (is_diff) {
                ...
                printf("PREVIOUS FILE\t==>%s\nsize: %ld\taccess: %ld\tmodify: %ld\nuid: %d\tgid: %d\n", base_entry->path,
                       base_entry->status.st_size, base_entry->status.st_atime, base_entry->status.st_mtime,
                       base_entry->status.st_uid, base_entry->status.st_gid);
                printf("CURRENT FILE\t==>%s\nsize: %ld\taccess: %ld\tmodify: %ld\nuid: %d\tgid: %d\n\n", check_entry->path,
                       check_entry->status.st_size, check_entry->status.st_atime, check_entry->status.st_mtime,
...
void traverse(const char *dir_name, void (*func)(struct file_info *)) {
    ...
    while ((entry = readdir(cur_dir)) != NULL) {
        bool is_period = !(strcmp(entry->d_name, ".") && strcmp(entry->d_name, ".."));
        if (!is_period) {
